...
show_help() {
    ...
    echo "  -l, --local [PATH]       使用本地源码编译 (可选指定路径，默认当前目录)"
    ...
    echo "  $0 -b                   # 更新并备份数据"
    echo "  $0 -l                   # 使用当前目录的源码编译"
    echo "  $0 --local /path/to/sui # 使用指定路径的源码编译"
}
...
parse_args() {
    ...
    if [[ "$USE_LOCAL_SOURCE" == true ]]; then
        ...
        # 验证是否为Sui项目
        if [[ ! -f "$LOCAL_SOURCE_PATH/crates/sui-node/Cargo.toml" ]]; then
...
download_code() {
    if [[ "$USE_LOCAL_SOURCE" == true ]]; then
        ...
        # 使用本地源码路径
        SUI_DIR="$LOCAL_SOURCE_PATH"
        cd "$SUI_DIR"
        
        log_info "使用本地源码: $SUI_DIR"
...
install_new_version() {
    ...
    # 复制新二进制文件
    sudo cp "$SUI_DIR/target/release/sui-node" /opt/sui/bin/
