...
impl TransactionExecutionApi {
    ...
    /// Execute a dry run of a transaction block with specific objects overridden.
    ///
    /// This method allows simulation of transaction execution with modified object states,
    /// which is useful for testing, MEV analysis, and debugging scenarios.
    ///
    /// # Arguments
    /// * `tx_bytes` - BCS serialized transaction data
    /// * `override_objects` - BCS serialized vector of (ObjectID, Object) pairs to use instead of on-chain objects
    ///
    /// # Returns
    /// A complete dry run response including effects, events, object changes, and balance changes
    /// calculated using the overridden objects.
