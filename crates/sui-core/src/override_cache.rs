...
///
/// This structure provides a way to simulate transaction execution with modified object states
/// by checking a local cache first before falling back to the underlying loader.
///
/// # Fields
/// * `loader` - Reference to the underlying TransactionInputLoader
/// * `cache` - Vector of (ObjectID, Object) pairs that override objects from the loader
...
/// A cache wrapper for BackingStore that allows overriding specific objects.
///
/// This structure implements the BackingStore, BackingPackageStore, ChildObjectResolver,
/// ObjectStore, and ParentSync traits while providing object override capabilities.
///
/// # Fields
/// * `inner` - The underlying backing store implementation
/// * `cache` - Vector of (ObjectID, Object) pairs that override objects from the backing store
